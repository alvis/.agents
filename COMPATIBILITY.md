...
| PR review-publication guard | ✅ Native | ✅ Native | 🟡 Adapted | 🟡 Adapted | The coding global PreToolUse gate permits its exact receipt-bound publisher and rejects supported raw `gh pr`, protected REST, and protected GraphQL review/comment writes through direct commands, `rtk`, `rtk proxy`, `env`, `command`, and one literal shell `-c` wrapper. Grok uses its native top-level decision envelope; OpenCode V1 runs the authenticated receipt-bound hook and contract resource. Arbitrary aliases or scripts, nested runtimes, `curl`, SDK/MCP clients, unsupported wrappers, and OpenCode V2 are outside enforcement. Source: [review-publishing.md](plugins/coding/skills/pr/directions/review-publishing.md). |
