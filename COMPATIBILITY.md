...
| Session context payloads | ✅ Native | ✅ Native | 🧪 Experimental | 🧪 Experimental | Universal context loads at startup; [domain context](docs/architecture/harness-projections.md#conditional-domain-context) activates from deterministic request, operation, or role evidence and uses identity-bound receipts to avoid unchanged repeats. Grok relies on its explicit loader and caller-carried receipt because passive SessionStart output is ignored; model compliance and later third-party tool-output replacement remain experimental. OpenCode reconstructs active domains through `experimental.chat.system.transform`; unresolved sessions receive only identifier mapping. |
...
| Plan-exit guard | ✅ Native | ✅ Native | 🟡 Adapted | 🟡 Adapted | Claude validates injected plan input or its explicit plan-file path. Codex/T3 validates the current final response at Stop, after rendering, independently of permission mode. Grok validates the session plan beside its transcript before `exit_plan_mode`. OpenCode validates the current session file before `plan_exit` when that tool is enabled; it has no general cancellable Stop event. All five sections are required. Rejected Codex plans remain pending through acknowledgements until correction passes, with one corrective continuation and visible terminal failure. Native payload and adapter tests cover these paths. Codex exec runtime 0.148.0 passed live rejection → feedback → corrected-plan acceptance with reviewed hooks enabled for the invocation; the other three live paths remain unverified. |
| PR review-publication guard | ✅ Native | ✅ Native | 🟡 Adapted | 🟡 Adapted | The coding global PreToolUse gate permits its exact receipt-bound publisher and rejects supported raw `gh pr`, protected REST, and protected GraphQL review/comment writes through direct commands, `rtk`, `rtk proxy`, `env`, `command`, and one literal shell `-c` wrapper. Grok uses its native top-level decision envelope; OpenCode V1 runs the authenticated receipt-bound hook, contract, and review-template resources. Arbitrary aliases or scripts, nested runtimes, `curl`, SDK/MCP clients, unsupported wrappers, and OpenCode V2 are outside enforcement. Source: [review-publishing.md](plugins/coding/skills/pr/directions/review-publishing.md). |
...
| Conditional state recovery | ✅ Native | ✅ Native | 🟡 Adapted | 🟡 Adapted | Owned material work without a current checkpoint requests one repair per applicable turn; unchanged or unowned work creates no continuation. The checkpoint protocol is unverified in live harnesses. Grok's [pre-release public source](https://github.com/xai-org/grok-build/tree/bc7f02eddd3d84085849dc19ed216f11c23b0571/crates/codegen/xai-grok-shell/src) supports Stop continuation; stable-release consumption remains unverified. OpenCode exposes no cancellable Stop event, so the receipt is labelled advisory system context and creates no synthetic turn. |
...
| `coding:pr` skill | 🔌 Integration | 🔌 Integration | 🔌 Integration | 🔌 Integration | Requires authenticated GitHub tooling; supported review writes additionally pass the review-publication guard above. Source: [SKILL.md](plugins/coding/skills/pr/SKILL.md). |
