...
Require passing applicable focused and full suites, 100% statements, branches, functions, and lines in every selected runtime source file, passing focused compile-time cases, and no remaining owned pending markers. Report batches, files/cases added and removed, fixture migrations, focused/full commands, independent-review result, compiler-semantic cases, aggregate metrics, and all four per-source runtime metrics. If any applicable gate cannot pass, report the concrete blocker and do not claim completion.
