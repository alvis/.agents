...
- **Is this the best solution?** Apply `essential:references/working-attitude.md`. Search the repository before accepting new code, content, tests, helpers, types, fixtures, utilities, or constants. Reimplementing an available equivalent is a finding. Encourage abstractions that clarify responsibilities or enable reuse.
...
- Exactly one of `priority` and `kind` is non-null. A comment that claims a consequence carries a priority; one that does not carries a kind. [review-tone.md](review-tone.md) selects the marker semantics and [inline-review.md](../templates/inline-review.md) owns their rendering through [review-publication.ts](../scripts/review-publication.ts), so this field decides the marker and no judgement is left at render time.
