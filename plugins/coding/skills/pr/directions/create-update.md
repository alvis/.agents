...
- Before every push, unless the caller explicitly supplied `--no-verify`, run the applicable `pull_request` test and lint tasks through read-only `jj run` at exact revision anchors, using the available local toolchain and dependencies. Run the selected tip first as a canary, then every selected PR surface bottom-up through the tip. No project dependency or tool installation, upgrade, or authentication is needed for this local check. The parity action must not inventory or presence-check CI-declared environment, secret, or variable values before attempting a task. Only captured failure evidence that an exact test or lint command failed because an exact CI-declared variable is missing may open the per-surface exception described in [verify.md](verify.md). Ask only for those names, either for an explicit source and rerun or for approval of that exact revision and exact lexically sorted name list. A missing required secret is the only per-surface exception; never infer approval from another flag or caller, guess a source, pass an empty value, or push after any other local failure.
