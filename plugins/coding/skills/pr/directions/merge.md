...
- **Required**: PR numbers in bottom-to-top order unless the user explicitly states the order is unknown; verify and stop if the real stack is not exactly linear. Bind each bare number's namespace through [resolve.md](resolve.md) first: a `stack` resolution belongs to GitHub's atomic stack merge in [github-stacks.md](../references/github-stacks.md#merge-and-navigate), not to this bottom-up loop.
- **Optional**:
