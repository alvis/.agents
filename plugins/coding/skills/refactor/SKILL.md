...
2. **Restructure** (focus `structure` or `all`). Apply design patterns per the standards, reduce function complexity, extract reusable helpers, and run tests continuously so no functionality breaks. When a desired domain basename collides with an existing file, or a file exceeds the project's `max-lines` threshold, apply the Two-Stage Rule before any ad-hoc split: Stage 1 — extract logic into another file as a genuinely reusable helper, preferred when more than one caller shares it; Stage 2 — when the desired entry would exceed the threshold, keep `<domain>.ts` as the thin re-export/orchestrator and move short-named sub-domains into `<domain>/*.ts` (the folder provides the context). Never derive a filename from the main export (`compute-similarity.ts` for `computeSimilarity()`), or split into arbitrary sibling files (`similarity-vector.ts`) when the nested entry pattern applies. The full rule and examples live in the coding plugin's `standards/file-structure/` under "Domain Collisions and Long-File Decomposition."
