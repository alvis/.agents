...
- All affected ancestors are UNMERGED on origin — else → [merged.md](./merged.md)
...
| Symptom | Action |
|---|---|
| `jj absorb` rejects an ancestor (immutable) | That ancestor is on a protected revset; it's likely already merged → [merged.md](./merged.md). |
...
| `jj blame` shows the change itself (`@`) | The hunk is genuinely new — it belongs as a new commit, not a retrospective. Surface and let user decide: keep on `@` and route to [save.md](./save.md). |
