...
- **FUNC-ARCH-06**: Options parameterize, add, or omit stages of one shared implementation. Put independently orchestrated pipelines in separate functions; a bounded adapter/proxy may select and delegate among them, adapting the boundary only when needed. Use the [rule guide](rules/func-arch-06.md) to distinguish execution selectors from domain facts regardless of naming, placement, or helper extraction; confirmed violations fail acceptance even when tests pass, without a general-policy waiver.
