...
- DO NOT let options select independently orchestrated pipelines, including through helpers; adding or omitting shared-pipeline stages is allowed, as is bounded adapter dispatch under [FUNC-ARCH-06](rules/func-arch-06.md). A confirmed violation fails acceptance even when tests pass and cannot be waived by the general exception policy [`FUNC-ARCH-06`]
...
| `FUNC-ARCH-06` | Options select separate pipelines outside a bounded adapter | `if (options.mode === "rebuild") { clearIndex(); scanAll(); } else { readCursor(); applyChanges(); }` |
