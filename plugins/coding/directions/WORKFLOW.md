...
- At an actual `jj` operation, read the shared setup and safety sections of
  [the `jj` guide](jj.md), then only the selected situation's recipe. Reuse
  those instructions for later operations in the same task; recheck repository
  state when the operation requires it.
...
Select applicable standards from `coding:standards/INDEX.md`; apply them under
`essential:directions/standards.md`. Reuse an already-read protocol; revisit
selection when the artifact or action changes.

## While Coding

Before writing code, apply the lean-work ladder, minimum-change rules, and
non-negotiable exceptions in `essential:references/working-attitude.md`.
