...
`skills/commit/SKILL.md` carries commit, branch, and local-history directions; the action references under `skills/pr/directions/` carry pull-request directions. `standards/git/` scans rendered PR messages and implementation-diff size and composition; `skills/pr/templates/message.md` owns the PR body shape, [the review templates](skills/pr/SKILL.md) own review presentation, and `skills/pr/scripts/review-publication.ts` consumes those templates and owns review assessment, approval, and publication. Other `standards/` directories carry implementation-scannable policy that `lint` and reviewers enforce; React and Web route their standards through the same executors.
