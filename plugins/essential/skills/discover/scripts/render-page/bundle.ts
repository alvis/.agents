...
export interface PageAssets {
  ...
  /**
   * every board of the run, when this board is one of several.
   *
   * it arrives here rather than in the board's own data because it is one
   * fact about the run, not fifteen facts about fifteen boards: the CLI reads
   * the run's set file once and hands the same list to each render.
   */
  set?: BoardSet;
}
