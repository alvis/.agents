...
import { readSources, sourcesOf } from "./source-file.ts";
...
describe("fn:sourcesOf", () => {
  ...
  it("should find a file a container nests, not only a top-level one", () => {
    // E-118 — the walk was flat, so a picture behind a disclosure was never
    // resolved and never inlined; the board then referenced a file it had not
    // read, and nothing anywhere said why
    expect(
      sourcesOf(
        board([
          { type: "disclosure", summary: "Detail", blocks: [{ type: "image", src: "deep.png" }] },
        ]),
      ),
    ).toStrictEqual([
      { src: "deep.png", type: "image", path: "sections[0].blocks[0].blocks[0]" },
    ]);
  });

  it("should keep descending however deep the containers go", () => {
    expect(
      sourcesOf(
        board([
          {
            ...
            summary: "Outer",
            blocks: [
              { type: "disclosure", summary: "Inner", blocks: [{ type: "svg", src: "a.svg" }] },
            ],
          },
        ]),
      ).map(({ path }) => path),
    ).toStrictEqual(["sections[0].blocks[0].blocks[0].blocks[0]"]);
  });
  ...
  // this walk runs before anything validates the board, so it meets shapes the
  // renderer would refuse by name. It must hand them on rather than crash: a
  // `blocks` that is not a list once threw a TypeError naming a function body,
  // in place of the refusal that names the section
  it.each([
    ["a section", { sections: [{ blocks: "not-an-array" }] }],
    [
      "a disclosure",
      { sections: [{ blocks: [{ type: "disclosure", blocks: "not-an-array" }] }] },
    ],
  ])("should walk past blocks that are not a list, in %s", (_, data) => {
    expect(sourcesOf(data as unknown as PageData)).toEqual([]);
  });
});
