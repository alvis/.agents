...
Read this when creating, accepting, or superseding a `decisions/` child or a durable decision record. It extends the decision statuses (`proposed|accepted|rejected|superseded`) with causal metadata and the sweep that keeps execution state honest after a decision changes.
