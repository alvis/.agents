...
Read this before performing main-agent state writes — the core contract carries
...
defaults). Before the first main-agent state write in a session, run the
...
still owns. `contended` means a live foreign main agent owns the stream —
...
After the main agent completes
...
`requires_ignore`. The resolver owns the mechanical bootstrap; the main agent
alone may request it while holding the lease. It creates the work
...
paths in `bootstrap_existing`; the main agent adds created paths to the combined
`generated_files` manifest.

## Writing under the lease

Perform every main-agent state write through
...
rename in one call — so a working main agent cannot expire its own lease by
working, and a main agent that lost the lease gets a hard error before the
