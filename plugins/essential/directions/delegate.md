...
- **Parallel first.** Map the task set as a dependency graph, drawing an edge only where one task truly needs another's output; batch edge-free tasks into one dispatch request and serialize only along real edges.
...
and effort fields, and a skill with no intelligence requirement stays eligible.
