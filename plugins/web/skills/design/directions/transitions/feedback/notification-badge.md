...
2. Keep the visual badge decorative with `aria-hidden="true"`. Update the trigger's `aria-label`, `aria-pressed`, and polite status text in one `render()` call so visible and announced state cannot diverge.
3. Drive visibility from one boolean and `data-visible`. Adapt the count and labels to real unread state instead of deriving semantics from opacity or transform.
4. Import the [shared motion asset](assets/transitions/motion.css) once. Keep the `motion-reduce` utilities so the badge changes immediately without travel or scale while the same unread state remains visible and announced.
5. Scope listeners with `AbortController`, retain the cleanup returned by `mount(root)`, and call it before unmount or remount.

## Verify

Toggle rapidly and confirm the count never shifts the trigger, the label and live text describe the same state, and reduced motion preserves both outcomes without movement. After cleanup, clicking the trigger must do nothing.
