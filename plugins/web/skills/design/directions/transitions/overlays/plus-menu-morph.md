...
4. Separate semantic and visual state. `aria-expanded` records the requested state, `hidden` plus `inert` plus `aria-hidden` controls menu exposure, and the morph container's `data-state` drives size, radius, content, and plus-icon animation. Expose content before the opening frame; make it inert before the exit; hide it only after the compact shape settles.
5. Grow width, height, and border radius from the fixed corner while the content reveals through opacity, translate, scale, and filter and the plus rotates. Keep each property list explicit, use shared tokens, and retain the non-animated reduced-motion end states.
...
7. Cancel pending timeouts and frames on every reversal, increment `stateVersion`, and reject stale close callbacks. Settle an in-flight state immediately when reduced motion becomes active. Cleanup must abort listeners, clear and invalidate work, restore trigger labeling and ARIA, reset roving tabindex, and leave content hidden and inert.
...
- Opening grows up and left inside the reserved footprint, rotates the plus to a close mark, exposes the menu, and causes no surrounding layout shift.
...
- Reverse direction during both entry and exit; only the latest request may determine visibility, shape, labels, and ARIA state.
- Enable reduced motion during a transition and confirm the expanded or compact final state appears immediately without residual blur or transform.
- Run cleanup while open and while closing; no timer, frame, listener, focusable hidden item, stale label, or expanded surface remains.
