...
1. Give each line `text-reveal-line` and a zero-based `--line-index`. Preserve heading and paragraph semantics and insert dynamic Unicode copy with `textContent`.
2. Merge the utilities after [the shared motion tokens](assets/transitions/motion.css). Calculate the last start as highest index × 40ms, then add the slow duration. Limit or regroup lines when that total no longer reads as one entrance.
3. Show by removing `hidden`, clearing exit styling, removing and re-adding the line utility around one forced layout. Hide by fading once and applying native `hidden` after the computed fast duration.
4. Treat `isShown` as intended final state and clear the old hide timer before every action. Keep `aria-live="polite"` only when restoring the production message should announce it as one unit.
5. A live reduced-motion change clears the timer and immediately applies `hidden` from `isShown`. Cleanup aborts listeners, removes the media-query listener, clears the timer, and removes transient exit styling.

## Verify

Test initial sequence, hide, replay, rapid hide/replay, calculated completion, localized multiline copy, live-region behavior, reduced motion during dismissal, native removal from layout and accessibility tree, and cleanup/remount.
