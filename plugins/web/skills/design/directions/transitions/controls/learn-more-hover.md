...
3. Preserve the explicit `translate` and `rotate` transition lists and adapt the distance, angle, and duration together. Browser pseudo-states and native link semantics own the complete interaction, so do not add a hover flag, timer, replay state, or cleanup path.
...
5. Keep every reduced-motion class. A live preference change returns translation and rotation to the stable chevron immediately while the focus ring, destination, and activation stay intact.
...
```html
<section class="grid min-h-40 place-items-center rounded-2xl bg-slate-950 p-8 text-white">
  <a href="#learn-more-destination" class="group inline-flex min-h-11 items-center gap-2 rounded-md text-lg font-semibold outline-none ring-sky-400/50 focus-visible:ring-4">
    Learn more
