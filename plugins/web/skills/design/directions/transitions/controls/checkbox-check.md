...
1. Keep the native checkbox inside its text label. Its `checked` state remains the only state owner, so form submission, reset, validation, accessible naming, label clicks, and Space activation work without JavaScript.
2. Place the custom box and check SVG after the visually hidden input. Drive border, fill, and focus-ring styles from the label group's checked and focus-visible selectors; keep the visual box and SVG `aria-hidden` so they do not duplicate the control.
3. Normalize the check path with `pathLength="1"`, set dash array and offset to `1`, and transition the offset to `0` only after checked state changes. The shorter box-color duration establishes the state before the longer stroke draw. Adapt the path freely while preserving the normalized length contract.
...
5. Keep explicit transition-property lists and every reduced-motion class. A live preference change removes interpolation while the checked fill and complete path still reflect the current native state; there is no timer, replay flag, or cleanup hook to maintain.
