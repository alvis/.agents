...
> The examples below use the augmentation template format. For the full structured (global-to-local) approach, see [Structured prompt examples](#structured-prompt-examples) at the end of this section.
...
These demonstrate the global-to-local structured approach. Each prompt layers elements from broadest context to finest detail, using the 7-element framework (Subject, Composition, Context, Medium, Style, Vibe, Attributes).

#### Photorealistic — cinematic portrait (structured)

```
Cinematic fashion editorial photograph captured outdoors during golden hour.
Subject: a young woman in an oversized red hooded nylon garment, relaxed
three-quarter pose, head turned toward camera with direct eye contact.
Environment: open untamed field of tall wild grass, no urban elements.
Framing: camera very low to ground, shooting upward; softly blurred
wheat-like plants dominate the foreground, forming diagonal lines that
naturally frame the subject.
Lighting: warm golden-hour backlight casting a soft glow; sky fading from
warm yellow near the horizon to gentle blue above.
Camera: shallow depth of field, face sharp, foreground and background
blurred; 50mm lens.
Vibe: poetic, powerful, alive — raw fashion in real wind, no studio
lighting or artificial posing.
Attributes: natural wind inflates and wraps fabric; hair blows across face
partially obscuring it; subtle film grain; warm amber color temperature.
Avoid: studio polish; staged poses; stock-photo vibe; oversaturated color.
```

#### 3D render — designer toy lookbook (structured)

```
Digital 3D fashion lookbook render of six anthropomorphic characters.
Subject: chibi-proportioned figures with oversized heads and compact
bodies; identical minimalist faces with small glossy bead-like black
eyes, no nose, thin minimal mouth line.
Environment: solid light grey background and matching floor; generous
negative space around figures.
Composition: clean 3x2 grid layout; figures slightly smaller in frame;
personality expressed through posture, not facial expression.
Materials: matte plush body texture with subtle metallic shine on layered
chains and accessories; each figure wears distinct layered streetwear
(hoodies, puffer vests, cargo pants, chunky sneakers).
Lighting: soft frontal diffused lighting; gentle grounded shadows.
Camera: straight-on angle; even framing across all six figures.
Vibe: playful, collectible, contemporary — designer vinyl toy aesthetic.
Color system: muted earth tones with one accent color per figure (dusty
rose, sage, terracotta, slate blue, cream, charcoal).
Avoid: glossy plastic finish; harsh shadows; cluttered backgrounds.
```
...
Reference: style image (via --reference)
